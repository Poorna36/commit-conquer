...
/**
 * In-memory store for password reset request timestamps.
 * Used to enforce a rate limit (cooldown) on password reset emails to prevent spam
 * and abuse. In a distributed production environment, this should be moved to Redis.
 * Key: normalized email address, Value: timestamp of the last request (ms).
 */
...
export const AuthService = {
  ...
  async requestPasswordReset(email: string): Promise<{ reset_token: string }> {
    ...
    // Enforce a 60-second cooldown between password reset requests for the same email.
    // This prevents malicious actors from spamming users with reset emails and 
    // provides basic rate limiting for the endpoint.
    const now = Date.now();
    const lastRequest = resetRequestCooldowns.get(emailKey);
    ...
    if (lastRequest && now - lastRequest < COOLDOWN_MS) {
      ...
      throw new ServiceError(
        "RATE_LIMIT_EXCEEDED",
        `Please wait ${waitTime} seconds before requesting another password reset.`
      );
    }
    
    // Record the timestamp of this request to enforce the cooldown on subsequent calls.
    resetRequestCooldowns.set(emailKey, now);
